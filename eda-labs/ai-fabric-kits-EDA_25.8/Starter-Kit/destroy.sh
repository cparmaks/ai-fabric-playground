...
# Default file paths
TOPOLOGY_FILE="ai-fabric-1tier.yaml"
TOPOLOGY_DATA="clab-ai-fabric-1tier/topology-data.json"

# Prompt user for EDA URL
read -p "Enter EDA URL (e.g., https://<EDA URL with port ID>): " EDA_URL
...
destroy_all() {
    log_info "Starting cleanup and destruction..."

    if [[ -z "$EDA_URL" ]]; then
        log_error "EDA URL is required for cleanup. Use -e or --eda-url option."
        exit 1
    fi
    ...
    if [[ -f "$TOPOLOGY_FILE" ]]; then
        if ! clab destroy -t "$TOPOLOGY_FILE"; then
